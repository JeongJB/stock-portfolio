...
- **비용·brute force 가드레일** (WAF 대체): `ApiThrottleRateLimit=5 / BurstLimit=10 / ApiQuotaPerDay=2000` 으로 Usage plan 강화 + `LambdaReservedConcurrency` (default 0 = 미설정, 한도 증액 후 1 로 활성화) + `AWS::Budgets::Budget` 월 $5 (ACTUAL 80% / FORECASTED 100%) 이메일 알림. WAF 미사용은 의도적 — 1인용에서 비용 0 으로 동등 효과. CloudFront GeoRestriction(KR-only) + Basic Auth 와 함께 4-layer 방어.
...
- **Lambda Reserved Concurrency 비활성 상태**: 신규 AWS 계정의 Lambda concurrent execution quota 가 10 (정상 prod 1000) 인 경우, `ReservedConcurrentExecutions=1` 을 설정하면 unreserved pool 이 9 가 되어 AWS hard floor 10 미만으로 거부된다. 이를 회피하기 위해 `LambdaReservedConcurrency` 디폴트를 0(=속성 부재)으로 두고, Conditional 로 0 일 때 `AWS::NoValue` 처리. 한도를 1000 으로 증액(Service Quotas: Lambda L-B99A9384)한 뒤 deploy 시 `--parameter-overrides LambdaReservedConcurrency=1` 또는 디폴트 변경으로 활성화. 그 동안은 throttle/quota + Budgets 3-layer 만으로 운영.
