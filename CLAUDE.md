...
| infra: Lambda SnapStart | `BackendFunction` 에 `SnapStart: ApplyOn: PublishedVersions` + `AutoPublishAlias: live` 두 줄 추가. SAM 이 매 배포마다 새 버전 publish → snapshot 생성 → `live` alias 갱신 → API Gateway integration / Lambda permission 자동으로 alias ARN 으로 박음. Spring Cloud Function `FunctionInvoker` 가 init 단계에서 ApplicationContext 부팅 → 그 상태가 snapshot 에 박혀 콜드 스타트 5~10s → 1~2s 단축. priming 코드 추가 없음. `ReservedConcurrentExecutions` (함수 단위) 와 호환. 알람 `FunctionName` dimension 은 모든 버전/alias 합산이라 변경 불필요. 첫 SnapStart 활성화 배포만 snapshot 빌드로 평소보다 5~15분 더 걸리고, 이후 배포는 함수 코드/설정 변경이 없으면 새 버전 발행 자체를 건너뛰어 snapshot 재생성 비용 없음. |
