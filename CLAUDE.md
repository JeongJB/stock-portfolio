...
- **콜드 스타트**: **Lambda SnapStart 적용** (`SnapStart: ApplyOn: PublishedVersions` + `AutoPublishAlias: live`). SAM 이 매 배포마다 새 버전 publish + `live` alias 갱신 + API Gateway integration 을 alias ARN 으로 자동 박제. Spring Cloud Function 의 init 단계 ApplicationContext 부팅이 snapshot 에 박혀 콜드 스타트 5~10s → 1~2s. AOT 는 미적용 (SnapStart 만으로 충분).
...
- **SnapStart + AutoPublishAlias**: SAM 이 `${BackendFunction}` 참조를 함수 이름으로 풀고(alias 가 아닌), 함수 events (API Gateway) 와 permission 만 자동으로 `${BackendFunction}:live` 로 박는다. 즉 알람의 `FunctionName` dimension 등 함수 단위 자원은 그대로 모든 버전/alias 합산을 본다 — alias-specific 메트릭이 필요하면 `Resource` dimension 을 별도 추가해야 함 (1인용엔 합산이 더 안전). 첫 SnapStart 활성화 배포는 snapshot 빌드로 평소보다 5~15분 더 걸리고, 이후 배포는 함수 코드/설정 변경이 없으면 새 버전 발행 자체를 건너뛰어 snapshot 재생성 비용 없음. SnapStart 비활성화 시 평소 첫 호출이 5~10s 콜드 스타트로 돌아간다. 롤백 시엔 alias 가 자동으로 직전 안정 버전을 가리키므로 git revert + redeploy 만으로 충분.
- **SnapStart cold start 최적화 4-trap**: ① SnapStart deploy 시 init phase 가 **2회** 실행되며 **마지막 init 의 결과가 snapshot 에 박힌다** — in-memory 만 의존하는 cache 는 두 번째 init 이 외부 호출 실패하면 빈 상태로 박힘. 외부 의존 cache (예: 환율) 는 반드시 DDB 같은 영속 저장소로 박제해 두 번째 init 이 첫 번째 init 의 결과를 재사용하도록 해야 함 (`FxRateStore` 패턴). ② SnapStart restore 시 OS-level socket 은 모두 닫힘 — TLS handshake / connection pool 은 snapshot 에 안 박힘. SDK 의 lazy init (region/credential/endpoint cache) 만 박힘. ③ `org.crac.Resource.afterRestore` priming 은 invoke duration 만 보면 빨라 보이지만 **사용자 wall clock = Restore Duration + Invoke Duration** 이므로 wall clock 으로는 오히려 손해. `@PostConstruct` 만으로 충분. ④ `dynamoDbClient.describeTable` 을 priming 에 쓰려면 SAM IAM 정책에 `dynamodb:DescribeTable` 추가 필요 (기본 GetItem/Query 권한엔 없음).
- **KIS access token EGW00133 (1분당 1회)**: KIS `/oauth2/tokenP` 가 1분 내 재발급 거부. SnapStart 의 init phase 2회 실행 시 두 번째 init 이 새 토큰 발급 시도하면 EGW00133 받고 fallback 으로 폴스루. `KisAccessTokenManager` 의 DDB 박제 (P1-7) 가 정상 동작하면 두 번째 init 이 DDB hit 으로 회피. fxCache 도 같은 이유로 DDB 박제 (`FxRateStore`) 필요.
