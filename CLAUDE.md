...
- **비용·brute force 가드레일** (WAF 대체): `ApiThrottleRateLimit=5 / BurstLimit=10 / ApiQuotaPerDay=2000` 으로 Usage plan 강화 + `LambdaReservedConcurrency=1` 로 동시 실행 상한 + `AWS::Budgets::Budget` 월 $5 (ACTUAL 80% / FORECASTED 100%) 이메일 알림. WAF 미사용은 의도적 — 1인용에서 비용 0 으로 동등 효과. CloudFront GeoRestriction(KR-only) + Basic Auth 와 함께 4-layer 방어.
...
- **API throttle / Lambda concurrency 부수효과**: 5 RPS / burst 10 / 일 2000 + 동시 실행 1로 캡됐다. 정상 1인 사용엔 무관하나 본인이 두 탭에서 동시에 새로고침하거나 단기간 큰 query 폭이 발생하면 일시적 429/직렬화가 발생할 수 있음. 거슬리면 `ApiThrottleBurstLimit` 또는 `LambdaReservedConcurrency` 만 올려서 redeploy.
